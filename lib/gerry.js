...
exports.Gerry = ( function () {
    ...
    gproto.enable = function ( collect, lfn ) {
        ...
        for ( ; i < elen; ++i ) addLogListener.call( me, events[ i ], collect || me.collect );
        return me;
    };
    ...
    gproto.push = function ( arr, collect ) {
         var me = this
            , args = isArray( arr ) ? arr : aSlice.call( arguments ).slice( 1 )
            , alen = args.length
            , i = 0
            , evt = args[ 0 ]
            , evts = []
        ...
        for ( ; i < alen; evt = args[ ++i ] ) {
            ...
            evts.push( evt );
            addLogListener.call( me, evt, collect || me.collect );
        }
        return aPush.apply( me.events, evts );
    };
